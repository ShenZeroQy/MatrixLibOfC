...
Cal_state Matrix_init(Matrixd* mat, int row, int col, int f)
{
	...
	mat->col = col;
	mat->row = row;
	mat->data = (double*)malloc(col*row * sizeof(double));
	int i, j;
	switch (f)
	{
	case '0'://全0
...
Cal_state Matrix_copy(Matrixd* mat, Matrixd* copy)
{
	...
	copy->row = mat->row;
	copy->trans_flag = 0;
	copy->data = (double*)realloc(copy->data, mat->col*mat->row * sizeof(double));
	int i, j;
	for (i = 0; i < mat->row; i++)
		for (j = 0; j < mat->col; j++)
...
Cal_state Matrix_reshape(Matrixd* mat, int row, int col)//只有不重新分配地址的时候，才保留原数据
{
	...
	{
		mat->data = (double*)realloc(mat->data, row*col * sizeof(double));//enlarge memory
		s = 2;
	}
	else
	{
...
void Matrix_clr_trans_flag(Matrixd* mat)
{
	...
	if (mat->trans_flag)
	{
		double* tem = (double*)malloc(mat->col*mat->row * sizeof(double));
		int i, j;
		for (i = 0; i < mat->row; i++)
			for (j = 0; j < mat->col; j++)
...
void MatrixS_Up_Triangle_inverse(Matrixd* mat, Matrixd* inv)
{
	...
	Vector One_col;
	One_col.len = N;
	One_col.data = (double*)malloc(N * sizeof(double));
	Vector solu_col;
	solu_col.len = N;
	solu_col.data = (double*)malloc(N * sizeof(double));
	int i, j;
...
void Matrix_get_col(Matrixd* mat, Vector* col_vec, int col)
{
	//	//默认col_vec 已经分配了存储
	//	col_vec->len = mat->row;
	//	col_vec->data = (double*)realloc(mat->row, sizeof(double));
	//	int i;
	//	for (i = 0; i < mat->row; i++)
	//	{
